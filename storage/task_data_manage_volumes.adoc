...
NOTE: Les systèmes d'exploitation diffèrent dans leur gestion des volumes clonés. VMware ESXi traite un volume cloné comme une copie de volume ou un volume snapshot. Le volume sera un périphérique disponible pour créer un nouveau datastore. Pour plus d'informations sur le montage des volumes clonés et la gestion des LUN snapshot, consultez la documentation VMware sur https://techdocs.broadcom.com/["Montage d'une copie de datastore VMFS"^] et https://techdocs.broadcom.com/["Gérer les datastores VMFS en double"^].
