...
.Ce dont vous avez besoin, 8217;ll
* Si votre nœud de gestion n'est pas connecté à Internet, vous avez téléchargé le pack à l'emplacement suivant :
+
https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["Package de mise à niveau du logiciel Element (.tar.gz) pour les clusters de stockage SolidFire"^]
...
... Voir  H610S storage nodes to Element 12.3.x (phase 2) et terminez le processus pour chaque nœud.
... Réauthentifier si nécessaire et sélectionner *PUT ​/upgrades/{upseId}*.
... Sélectionnez *essayez-le*.
... Saisissez l'ID de mise à niveau de l'étape précédente dans *mise à niveau Id*.
...
. Téléchargez-le https://library.netapp.com/ecm/ecm_get_file/ECMLP2840740["Fichier JSON"^] (https://library.netapp.com/ecm/ecm_get_file/ECMLP2840740)[] Sur le site de support NetApp d'un ordinateur qui n'est pas le nœud de gestion, puis renommez-le en `metadata.json`.
